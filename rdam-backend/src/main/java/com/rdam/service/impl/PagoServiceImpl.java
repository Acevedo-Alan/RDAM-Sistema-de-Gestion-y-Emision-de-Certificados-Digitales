...
public class PagoServiceImpl implements PagoService {
    ...
    public SolicitudPagoResponse generarDatosPago(Integer solicitudId, Integer userId) {
        ...
        if (solicitud.getEstado() != EstadoSolicitud.PENDIENTE) {
            throw new EstadoInvalidoException(
                    "La solicitud no esta en estado PENDIENTE. Estado actual: " + solicitud.getEstado());
        }
        ...
        // Sanitizar URLs para evitar dobles slashes
        String frontendUrlClean = frontendUrl.replaceAll("/$", "");
        String backendUrlClean = backendUrl.replaceAll("/$", "");

        // Encriptar datos para el mock PlusPagos
        String montoEncriptado = cryptoService.encrypt(String.valueOf(montoCentavos));
        String urlSuccess = cryptoService.encrypt(
                frontendUrlClean + "/ciudadano/solicitudes/" + solicitudId + "?pago=success");
        String urlError = cryptoService.encrypt(
                frontendUrlClean + "/ciudadano/solicitudes/" + solicitudId + "?pago=error");
        String callbackSuccess = cryptoService.encrypt(backendUrlClean + "/api/pagos/webhook");
        String callbackCancel = cryptoService.encrypt(backendUrlClean + "/api/pagos/webhook");
    ...
    public void procesarWebhookPago(WebhookPagoRequest request) {
        ...
        // Validar estado
        if (solicitud.getEstado() == EstadoSolicitud.PAGADO || solicitud.getEstado() == EstadoSolicitud.PUBLICADO) {
        ...
        // Transicionar solicitud a PAGADO
        ...
        try {
            ...
            throw new com.rdam.service.exception.ConcurrenciaException(
                    "La solicitud ID " + solicitudId
                            + " esta siendo procesada por otro usuario. Intente nuevamente.",
                    ex);
